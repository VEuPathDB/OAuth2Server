 ...
 * @author rdoherty
 */
public class KeyStoreTrustManager extends X509ExtendedTrustManager {

  private static final Logger LOG = LogManager.getLogger(KeyStoreTrustManager.class);
  ...
  public KeyStoreTrustManager(Path keyStoreFile, String passPhrase) {
    try (InputStream fileStream = new FileInputStream(keyStoreFile.toAbsolutePath().toString())) {
      ...
      throw new RuntimeException("Couldn't initialize trust manager using key store file " + keyStoreFile);
    }
    catch (IOException | NoSuchAlgorithmException | CertificateException | KeyStoreException e) {
      throw new RuntimeException("Couldn't initialize trust manager using key store file " + keyStoreFile, e);
    }
  }
